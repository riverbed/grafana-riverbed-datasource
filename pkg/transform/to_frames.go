...
func framesFromTimeSeries(q api.ResponseQuery) data.Frames {
    ...
    for k := range tsMap { tsKeys = append(tsKeys, k) }
    ...
    for i, k := range tsKeys { times[i] = tsMap[k] }
    ...
    for _, dp := range q.Data {
        ...
        for _, m := range q.Meta.Metrics {
            ...
            if nMetrics > 1 && nKeys > 1 {
                ...
                if lbls != nil && len(lbls) > 0 && nKeys > 1 {
                    name = m.Name
                }
...
func framesFromSummary(q api.ResponseQuery) data.Frames {
    ...
    for i := range cols {
        if cols[i].isKey { strCols[i] = []string{} } else { numCols[i] = []*float64{} }
    }
    for _, dp := range q.Data {
        for i, c := range cols {
            if c.isKey {
                var sval string
                if v, ok := dp.Keys[c.id]; ok { if s, ok := v.(string); ok { sval = s } else { b, _ := json.Marshal(v); sval = string(b) } }
                strCols[i] = append(strCols[i], sval)
            } else {
                var f *float64
                if v, ok := dp.Metrics[c.id]; ok { f = v }
