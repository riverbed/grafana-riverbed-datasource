...
func NewDatasource(s backend.DataSourceInstanceSettings) (instancemgmt.Instance, error) {
	...
	// set defaults
	if jd.PollMaxSeconds <= 0 {
		jd.PollMaxSeconds = 60
	}
	if jd.PollIntervalMs <= 0 {
		jd.PollIntervalMs = 500
	}
	// Clamp poll settings to safe ranges
	if jd.PollMaxSeconds < 5 {
		jd.PollMaxSeconds = 5
	}
	if jd.PollMaxSeconds > 600 {
		jd.PollMaxSeconds = 600
	}
	if jd.PollIntervalMs < 100 {
		jd.PollIntervalMs = 100
	}
	if jd.PollIntervalMs > 60000 {
		jd.PollIntervalMs = 60000
	}
	...
	if jd.MaxPageRequests <= 0 {
		jd.MaxPageRequests = 20
	}
	if jd.MaxRows <= 0 {
		jd.MaxRows = 2_000_000
	}
	// Clamp pagination safety caps
	if jd.MaxPageRequests > 100 {
		jd.MaxPageRequests = 100
	}
	if jd.MaxPageRequests < 1 {
		jd.MaxPageRequests = 1
	}
	if jd.MaxRows > 10_000_000 {
		jd.MaxRows = 10_000_000
	}
...
func (d *Datasource) query(ctx context.Context, pCtx backend.PluginContext, q backend.DataQuery) backend.DataResponse {
	...
	resp, err := d.proxyRequestWithHeaders(ctx, http.MethodPost, path, b, map[string]string{
		"X-Request-Timeout": func() string {
			if d.requestTimeoutSeconds > 0 {
				return fmt.Sprintf("%d", d.requestTimeoutSeconds)
			} else {
				return ""
			}
		}(),
...
func (d *Datasource) CheckHealth(ctx context.Context, _ *backend.CheckHealthRequest) (*backend.CheckHealthResult, error) {
	...
	defer func() { _ = queriesResp.Body.Close() }()
