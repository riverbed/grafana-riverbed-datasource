...
func (s *ClientCredentialsSource) Token(ctx context.Context) (*Token, error) {
    ...
    if s.Clock == nil { s.Clock = realClock{} }
    ...
    if s.Scope != "" { form.Set("scope", s.Scope) }
    ...
    if resp.StatusCode < 200 || resp.StatusCode >= 300 { return nil, errors.New("token request failed: " + resp.Status) }
    var tr struct {
        AccessToken string `json:"access_token"`
        ExpiresIn   int    `json:"expires_in"`
    }
    if err := json.NewDecoder(resp.Body).Decode(&tr); err != nil { return nil, err }
    if tr.AccessToken == "" { return nil, errors.New("empty access_token") }
    exp := s.Clock.Now().Add(time.Duration(tr.ExpiresIn) * time.Second)
    if tr.ExpiresIn == 0 { exp = s.Clock.Now().Add(3600 * time.Second) }
